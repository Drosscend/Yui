...
exports.run = async (client, message, args) => {
    ...
    message.channel.send("<:picture:605752181173256202> Génération de l'image...").then(msg => {
        get(`https://eclyssia-api.tk/api/v1/triggered?url=${profilepic}`, {
            responseType: "arraybuffer"
        })
        .then(res =>
            message.channel.send("<:picture:605752181173256202> Image **triggered** générée par **eclyssia-api.tk**:",{
                file: { attachment: res.data, name: "image.gif" }
            })
            )
        .then(() => msg.delete())
        .catch(err => {
